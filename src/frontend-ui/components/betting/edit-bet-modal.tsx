...
export default function EditBetModal({ bet, isOpen, onClose, onSave }: EditBetModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // If bet is won or lost, recalculate returns and profit/loss
      if (formData.status === 'Won' || formData.status === 'Lost') {
        if (formData.status === 'Won') {
          // For winning bets, returns = stake * odds
          updatedReturns = (formData.stake || 0) * (formData.odds || 0);
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto">
      <div className="flex min-h-full items-center justify-center p-4 text-center">
        ...
        {/* Modal content */}
        <div className={`relative w-full max-w-2xl transform rounded-lg ${getModalBg()} ${getBorderColor()} border p-6 text-left shadow-xl transition-all`}>
          {/* Close button */}
          <button
            onClick={onClose}
          ...
          <form onSubmit={handleSubmit}>
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              ...
              {/* Scheduled Race Time */}
              <div>
                <label htmlFor="scheduled_race_time" className={`block text-sm font-medium ${getMutedTextColor()} mb-1`}>
                  Race Time
                </label>
                <input
                  type="datetime-local"
                  name="scheduled_race_time"
                  id="scheduled_race_time"
                  value={formData.scheduled_race_time ? new Date(formData.scheduled_race_time).toISOString().slice(0, 16) : ''}
                  onChange={handleChange}
                  className={`block w-full rounded-md border ${getInputBorder()} ${getInputBg()} ${getInputTextColor()} px-3 py-2 text-sm focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500`}
                />
              </div>
              ...
              {/* Bet Type */}
              <div>
                <label htmlFor="bet_type" className={`block text-sm font-medium ${getMutedTextColor()} mb-1`}>
                  Bet Type *
                </label>
                <select
                  name="bet_type"
