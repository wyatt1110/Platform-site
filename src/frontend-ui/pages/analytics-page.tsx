...
export default function AnalyticsPage() {
  ...
  const [activeChartType, setActiveChartType] = useState('profitChart');
  ...
  const getHeaderBg = () => theme === 'dark' ? 'bg-gray-900 border-gray-800' : theme === 'racing' ? 'bg-charcoal-700 border-charcoal-800' : 'bg-white border-gray-200';
  ...
  const getActiveButtonBg = () => theme === 'racing' ? 'bg-racing-100 text-racing-600' : theme === 'dark' ? 'bg-blue-900 text-blue-400' : 'bg-blue-50 text-blue-600';
  const getInactiveButtonBg = () => theme === 'dark' ? 'text-gray-400 hover:bg-gray-700' : theme === 'racing' ? 'text-gray-300 hover:bg-charcoal-700' : 'text-gray-500 hover:bg-gray-50';
  const getChartBg = () => theme === 'dark' ? 'bg-gray-700' : theme === 'racing' ? 'bg-charcoal-700' : 'bg-gray-100';
  
  // New theme-aware chart color functions
  const getPositiveColor = () => theme === 'dark' ? '#4ade80' : theme === 'racing' ? '#4ade80' : '#22c55e';
  const getNegativeColor = () => theme === 'dark' ? '#f87171' : theme === 'racing' ? '#f87171' : '#ef4444';
  const getProfitLossColor = (value: number) => value >= 0 ? getPositiveColor() : getNegativeColor();
  const getChartLineColor = () => theme === 'dark' ? '#60a5fa' : theme === 'racing' ? '#38bdf8' : '#3b82f6';
  const getChartSecondaryLineColor = () => theme === 'dark' ? '#34d399' : theme === 'racing' ? '#34d399' : '#10b981';
  ...
  const barChartOptions = useMemo(() => {
    ...
    if (activeProfitMetric === 'track') {
      ...
      // Sort by absolute profit value for visualization
  ...
  const roiChartOptions = useMemo(() => {
    ...
    if (activeRoiMetric === 'track') {
      ...
      // Calculate ROI per track
      const trackROI: Record<string, { roi: number; sampleSize: number; totalStake: number }> = {};
      ...
      settledBets.forEach(bet => {
        ...
        if (bet.track_name && bet.track_name.includes('/')) {
          ...
          tracks.forEach(track => {
            if (!trackROI[track]) {
              trackROI[track] = { roi: 0, sampleSize: 0, totalStake: 0 };
            }
            trackROI[track].sampleSize += 1;
            trackROI[track].totalStake += stakePerTrack;
            // Update running ROI
            trackROI[track].roi = trackROI[track].totalStake > 0 
              ? (profitPerTrack / stakePerTrack) * 100  // Calculate ROI as percentage
              : 0;
          });
        } else {
          ...
          // Calculate running ROI total
          trackROI[track].roi = trackROI[track].totalStake > 0 
            ? (profitLoss / stake) * 100  // Calculate ROI as percentage
            : 0;
        }
      });
      
      // Recalculate ROI for each track using the total profit and stake
      Object.keys(trackROI).forEach(track => {
        const betsByTrack = settledBets.filter(bet => bet.track_name === track || 
                                           (bet.track_name && bet.track_name.includes('/') && 
                                            bet.track_name.split('/').map(t => t.trim()).includes(track)));
        
        let totalProfit = 0;
        let totalStake = 0;
        
        betsByTrack.forEach(bet => {
          if (bet.track_name === track) {
            totalProfit += bet.profit_loss || 0;
            totalStake += bet.stake;
          } else {
            // For multi-track bets, divide profit and stake
            const tracks = bet.track_name!.split('/').map(t => t.trim()).filter(t => t);
            const trackCount = tracks.length;
            totalProfit += (bet.profit_loss || 0) / trackCount;
            totalStake += bet.stake / trackCount;
          }
        });
        
        trackROI[track].roi = totalStake > 0 ? (totalProfit / totalStake) * 100 : 0;
      });
      
      // Filter out tracks with too few bets (at least 2 for statistical relevance)
      ...
      // Get top 5 best and worst ROI
      bestData = [...filteredTrackROI].sort((a, b) => b.value - a.value).slice(0, 5);
      worstData = [...filteredTrackROI].sort((a, b) => a.value - b.value).slice(0, 5);
      ...
      // Sort by ROI value
      bestData = [...oddsRangeData].sort((a, b) => b.value - a.value);
      worstData = [...oddsRangeData].sort((a, b) => a.value - b.value);
      ...
      // Get top 5 best and worst ROI
      bestData = [...filteredJockeyROI].sort((a, b) => b.value - a.value).slice(0, 5);
      worstData = [...filteredJockeyROI].sort((a, b) => a.value - b.value).slice(0, 5);
      ...
      // Get top 5 best and worst ROI
      bestData = [...filteredTrainerROI].sort((a, b) => b.value - a.value).slice(0, 5);
      worstData = [...filteredTrainerROI].sort((a, b) => a.value - b.value).slice(0, 5);
      ...
      settledBets.forEach(bet => {
        ...
        if (bet.class_type.includes('/')) {
          ...
          classes.forEach(classType => {
            ...
            classROI[classType].sampleSize += 1;
            classROI[classType].totalStake += stakePerClass;
            classROI[classType].totalProfit += profitPerClass;
          });
        } else {
          if (!classROI[bet.class_type]) {
            classROI[bet.class_type] = { roi: 0, sampleSize: 0, totalStake: 0, totalProfit: 0 };
          }
          classROI[bet.class_type].sampleSize += 1;
          classROI[bet.class_type].totalStake += stake;
          classROI[bet.class_type].totalProfit += profitLoss;
        }
      });
      ...
      // Get top 5 best and worst ROI
      bestData = [...filteredClassROI].sort((a, b) => b.value - a.value).slice(0, 5);
      worstData = [...filteredClassROI].sort((a, b) => a.value - b.value).slice(0, 5);
    }

    // Combine data for visualization
    const combinedData = [...bestData, ...worstData];
    ...
    return {
      ...
      legend: {
        // Only show legend for non-odds metrics
        show: activeRoiMetric !== 'odds',
        data: [
          {
            name: 'Top 5 Best ROI',
            itemStyle: {
      ...
      grid: {
        ...
        right: '12%',
        bottom: 60,
        top: activeRoiMetric === 'odds' ? 40 : 50, // Less top space when no legend
        containLabel: true
      },
      xAxis: {
        type: 'value',
      ...
      yAxis: {
        type: 'category',
        data: activeRoiMetric === 'odds' 
          ? bestData.map(item => item.name)
          : [...bestData.map(item => item.name), ...worstData.map(item => item.name)],
        axisLine: {
          show: true,
          lineStyle: {
            color: theme === 'dark' || theme === 'racing' ? '#e2e8f0' : '#1a202c',
      ...
      series: activeRoiMetric === 'odds' ? [
        {
          name: 'ROI %',
          type: 'bar',
          data: bestData.concat(worstData),
          barWidth: '70%',
          itemStyle: {
            color: function(params: any) {
              return getProfitLossColor(params.value);
      ...
      ] : [
        {
          ...
          data: [...bestData.map(item => ({
            value: item.value,
            itemStyle: {
              color: getPositiveColor() // Always green for best ROI
            }
          })), ...worstData.map(() => null)],
          label: {
            show: true,
        ...
        },
        {
          ...
          type: 'bar',
          stack: 'total',
          data: [...bestData.map(() => null), ...worstData.map(item => ({
            value: item.value,
            itemStyle: {
              color: getNegativeColor() // Always red for worst ROI
            }
          }))],
          label: {
            show: true,
    ...
    <div>
      <main className="max-w-7xl mx-auto px-4 py-8 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col items-center justify-center mb-8">
          <div className={`bg-gray-800 rounded-xl shadow-lg p-4 w-full max-w-2xl transition-all transform hover:shadow-xl`}>
            <div className="flex flex-col space-y-4">
              ...
              {isDatePickerOpen && (
                <div className="bg-gray-700 rounded-lg p-4 animate-fade-in-down">
          ...
          <div className="mt-2 text-center">
            <span className="text-sm font-medium text-gray-400">
              Currently viewing: <span className="text-blue-400 font-semibold">{getDateRangeLabel()}</span>
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            <div className="group relative p-4 rounded-xl shadow-md hover:shadow-lg transition-all duration-300 ease-in-out 
                bg-gradient-to-br from-gray-800 to-gray-900 hover:-translate-y-1 cursor-pointer">
              <div className="absolute inset-0 rounded-xl opacity-0 group-hover:opacity-40 transition-opacity duration-300 ease-in-out"
                   style={{
                     background: `radial-gradient(circle at center, #3b82f620, transparent 70%)`,
                   }}></div>
              <div className="relative z-10">
                <div className="flex items-center justify-between mb-3">
                  <div className="p-2 rounded-lg bg-blue-900/40 text-blue-400 transform group-hover:scale-110 transition-transform duration-300 ease-in-out">
                    <TrendingUp className="h-4 w-4" />
                  </div>
                  {betStats.profitLoss > 0 && (
                    <div className="text-xs font-semibold px-2 py-1 rounded-full bg-green-100/20 text-green-500">
                      Profitable
                </div>
                  )}
                </div>
                <h3 className="text-xs font-medium text-gray-400 uppercase tracking-wider">Total Profit/Loss</h3>
            ...
            <div className="group relative p-4 rounded-xl shadow-md hover:shadow-lg transition-all duration-300 ease-in-out 
                bg-gradient-to-br from-gray-800 to-green-900/30 hover:-translate-y-1 cursor-pointer">
              <div className="absolute inset-0 rounded-xl opacity-0 group-hover:opacity-40 transition-opacity duration-300 ease-in-out"
                   style={{
              ...
              <div className="relative z-10">
                ...
                <h3 className="text-xs font-medium text-gray-400 uppercase tracking-wider">Win Rate</h3>
                <p className="text-xl font-bold mt-1 text-white tracking-tight group-hover:scale-105 transition-transform duration-300 ease-in-out">
            ...
            <div className="group relative p-4 rounded-xl shadow-md hover:shadow-lg transition-all duration-300 ease-in-out 
                bg-gradient-to-br from-gray-800 to-amber-900/30 hover:-translate-y-1 cursor-pointer">
              <div className="absolute inset-0 rounded-xl opacity-0 group-hover:opacity-40 transition-opacity duration-300 ease-in-out"
                   style={{
              ...
              <div className="relative z-10">
                <div className="flex items-center justify-between mb-3">
                  <div className="p-2 rounded-lg bg-amber-900/40 text-amber-400 transform group-hover:scale-110 transition-transform duration-300 ease-in-out">
                    <TrendingUp className="h-4 w-4" />
                  </div>
                </div>
