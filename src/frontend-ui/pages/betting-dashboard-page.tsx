...
const StatCard = ({ 
  ...
  const getAccentClasses = () => {
    const baseClasses = {
      primary: theme === 'dark' ? 'bg-green-900/40 text-green-400' : 
               theme === 'racing' ? 'bg-racing-900/40 text-racing-400' : 
               'bg-green-50 text-green-600',
      success: theme === 'dark' ? 'bg-green-900/40 text-green-400' : 
               theme === 'racing' ? 'bg-green-900/40 text-green-400' : 
               'bg-green-50 text-green-600',
      warning: theme === 'dark' ? 'bg-amber-900/40 text-amber-400' : 
               theme === 'racing' ? 'bg-amber-900/40 text-amber-400' : 
               'bg-amber-50 text-amber-600',
      danger: theme === 'dark' ? 'bg-red-900/40 text-red-400' : 
              theme === 'racing' ? 'bg-red-900/40 text-red-400' : 
  ...
  const getGradientBg = () => {
    const gradients = {
      primary: theme === 'dark' ? 'from-gray-800 to-gray-900' : 
               theme === 'racing' ? 'from-charcoal-800 to-charcoal-900' : 
               'from-white to-green-50',
      success: theme === 'dark' ? 'from-gray-800 to-green-900/30' : 
               theme === 'racing' ? 'from-charcoal-800 to-green-900/30' : 
               'from-white to-green-50',
      warning: theme === 'dark' ? 'from-gray-800 to-amber-900/30' : 
               theme === 'racing' ? 'from-charcoal-800 to-amber-900/30' : 
               'from-white to-amber-50',
      danger: theme === 'dark' ? 'from-gray-800 to-red-900/30' : 
              theme === 'racing' ? 'from-charcoal-800 to-red-900/30' : 
...
export default function BettingDashboardPage() {
  ...
  const chartOptions = useMemo(() => {
    ...
    const getChartLineColor = () => theme === 'dark' ? '#4ade80' : theme === 'racing' ? '#38bdf8' : '#22c55e';
      ...
      series: [
        {
          ...
          areaStyle: {
            color: {
              ...
              colorStops: [{
                offset: 0,
                color: theme === 'dark' || theme === 'racing' ? 'rgba(74, 222, 128, 0.5)' : 'rgba(34, 197, 94, 0.3)'
              }, {
                offset: 1,
                color: theme === 'dark' || theme === 'racing' ? 'rgba(74, 222, 128, 0.05)' : 'rgba(34, 197, 94, 0.05)'
              }]
            }
          },
          markLine: {
  ...
  const getTextColor = () => theme === 'dark' || theme === 'racing' ? 'text-white' : 'text-gray-900';
  const getMutedTextColor = () => theme === 'dark' ? 'text-gray-400' : theme === 'racing' ? 'text-gray-300' : 'text-gray-500';
  const getPrimaryButtonBg = () => theme === 'racing' ? 'bg-racing-600 hover:bg-racing-700' : 'bg-green-600 hover:bg-green-700';
    ...
    <div className="py-6 px-4 sm:px-6 lg:px-8">
      ...
      <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-8">
        ...
        <button 
          ...
          className={`inline-flex items-center justify-center rounded-md px-6 py-3 text-base font-medium text-white bg-green-600 hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-green-500 whitespace-nowrap hover:scale-105 transition-transform`}
        >
          + Add New Bet
        </button>
      </div>
      ...
      {/* Profit/Loss Chart */}
      <div className={`rounded-xl shadow-sm border p-5 ${getCardBg()} ${getBorderColor()} mb-6`}>
        <h3 className={`text-lg font-semibold mb-4 ${getTextColor()}`}>Profit and Loss</h3>
        <div className="h-[500px]">
          {chartData.length > 0 ? (
            ...
            <div className="text-center h-full flex items-center justify-center">
              <p className={`text-sm ${getMutedTextColor()}`}>No settled bets data available to display chart</p>
            </div>
          )}
        </div>
      </div>
      ...
      <div className={`${getCardBg()} rounded-xl shadow-sm border ${getBorderColor()} p-6`}>
        <h3 className={`text-lg font-semibold ${getTextColor()} mb-4`}>For You:</h3>
        <div className={`${theme === 'dark' ? 'bg-gray-900' : theme === 'racing' ? 'bg-charcoal-900' : 'bg-gray-50'} border ${getBorderColor()} rounded-lg p-6 text-center`}>
          <p className={`${getMutedTextColor()} mb-2`}>Personalized recommendations will appear here</p>
          <p className={`text-xs ${theme === 'dark' ? 'text-gray-500' : theme === 'racing' ? 'text-gray-400' : 'text-gray-400'}`}>Based on your betting history and market data</p>
        </div>
      </div>
